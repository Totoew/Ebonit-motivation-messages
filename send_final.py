...
def load_template(category):
   ...
   path = f"templates/{category_map[category]}.txt"
...
def get_video_attachment(vk, video_url):
   ...
   try:
       ...
       # Пример URL: https://vk.com/video-123456_789 или https://vk.com/video123456789_123
...
def preview_mode(vk_token):
   ...
   if not curators:
       print("❌ Не найдены vk_id кураторов в C3:C4")
       return
   ...
   for _, row in student_rows:
       ...
       if len(representatives[category]) < 2:
           ...
           representatives[category].append({
               'original_name': name,
               'message': (msg_text, video_url),  # Сохраняем кортеж
               'category': category
           })
   ...
   for cat in [1, 2, 3]:
       for rep in representatives[cat]:
           ...
           msg_text, video_url = rep['message']  # Распаковываем кортеж
           ...
           for curator_id in curators:
               try:
                   ...
                   total_sent += 2  # Считаем оба сообщения
               except Exception as e:
...
# --- Режим ОТПРАВКИ ---
def send_mode(vk_token):
   block_number = int(input("Номер блока: "))
   lesson_input = input("Диапазон домашек (например, 12-17): ")
   skip_input = input("Номера строк Excel, кому НЕ отправлять (через запятую, например: 12,15): ").strip()
   skip_rows = set()
   if skip_input:
       try:
           skip_rows = {int(x.strip()) for x in skip_input.split(',')}
       except:
           print("⚠️ Ошибка в номерах строк. Продолжаем без исключений.")


   df_full = pd.read_excel(EXCEL_FILE, header=None)
   headers = df_full.iloc[0]
   max_scores_row = df_full.iloc[6]
   # Ученики: строки 8 и далее → индексы 7, 8, ...
   student_rows = list(df_full.iloc[7:].iterrows())
   ...
   for original_idx, row in student_rows:
       excel_row_number = original_idx + 1  # потому что Excel нумерует с 1
       ...
       # Сбор баллов (аналогично preview)
       student_scores = []
       max_scores = []
       total_score = 0
       test_done_count = 0
       ...
       try:
           ...
           print(f"✅ Отправлено: {name} (строка {excel_row_number})")
           sent_count += 2  # Считаем оба сообщения
       except Exception as e:
...
if __name__ == '__main__':
   # Запрашиваем токен один раз при запуске
